...
function randomPickOk(randomPick, words, pickType) {
  if (pickType === "genrePicks") {
    return (words.length < 11 && !randomPick.name.match(/^[a-z]/ || /[a-z][a-z]./ || /,\s/) && notInPicksArray(randomPick, pickType));
  } else if (pickType === "directorPicks") {
    return (words.length > 1 && words.length < 5 && !randomPick.name.match(/^[a-z]/ || /[a-z][a-z]./ || /,\s/) && notInPicksArray(randomPick, pickType) && state.resultsMinusGPickIndices[randomPick.index]);
  } else if (pickType === "starPicks") {
    return (words.length < 4 && !randomPick.name.match(/^[a-z]/ || /[a-z][a-z]./ || /,\s/ || /&/ || /\"/) && notInPicksArray(randomPick, pickType) && state.resultsMinusGDPickIndices[randomPick.index]);
  }
}
